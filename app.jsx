...
const TAB_LABEL = { today: "Hôm nay", meals: "Thực đơn", shop: "Đi chợ", week: "Tuần" };
...
function applyConfig(cfg) {
  ...
  (cfg.people || []).forEach((p, i) => {
    ...
    PEOPLE[id] = {
      name: String(p["tên"] || id), short: String(p["viết tắt"] || id.slice(0, 2)).toUpperCase(),
      color, fg: "#fff", deep: shade(color, -70), soft: shade(color, 92),
    };
  ...
  (cfg.shifts || []).forEach((s) => {
    SHIFTS[Number(s["thứ"])] = { who: String(s["người"]), cap: Number(s["phút chuẩn"]) || 0 };
  });
  for (let d = 0; d < 7; d++) if (!SHIFTS[d]) SHIFTS[d] = { who: OWNER, cap: 0 };

  const kindOf = (f) => (f === "ngày" ? "daily" : f === "tuần" ? "weekly" : "monthly");
  ALL_TASKS = (cfg.tasks || []).filter((t) => t.id).map((t) => ({
    id: String(t.id), name: String(t["tên"]), room: String(t["khu vực"] || ""),
    min: Number(t["phút"]) || 0, kind: kindOf(String(t["tần suất"]).trim()),
  ...
  SEED_RECIPES = (cfg.recipes || []).filter((r) => r.id).map((r) => ({
    id: String(r.id), name: String(r["tên"]), type: String(r["loại"] || "Món chính"),
    ing: String(r["nguyên liệu"] || "").split(",").map((x) => x.trim()).filter(Boolean),
  }));
  const types = [];
  SEED_RECIPES.forEach((r) => { if (types.indexOf(r.type) < 0) types.push(r.type); });
  if (types.length) TYPES = types;

  ING_GROUP = {};
  (cfg.ingredients || []).forEach((i) => { ING_GROUP[String(i["tên"]).trim()] = String(i["nhóm"] || "Khác"); });

  const groups = {};
  (cfg.staples || []).forEach((s) => {
    const g = String(s["loại"] || "Khác");
    (groups[g] = groups[g] || []).push({ n: String(s["tên"]), c: String(s["nhóm"] || "Khác") });
  });
  STAPLE_GROUPS = Object.keys(groups).map((k) => ({ title: k, items: groups[k] }));
}

function catOf(ing) { return ING_GROUP[ing] || "Khác"; }
...
const byTime = (arr) =>
  [...arr].sort((a, b) => b.min - a.min || a.name.localeCompare(b.name, "vi"));
...
function planFor(week, date) {
  ...
  const tasks = ov?.tasks
    ? byTime(ov.tasks.map(taskById).filter(Boolean))
    : defaultTasks(date);
  return { tasks, assign: ov?.assign || {}, custom: !!ov?.tasks };
}
...
function hoursFor(week, date) {
  ...
  return { actual: h?.actual ?? cap, note: h?.note || "", edited: h?.actual != null && h.actual !== cap, cap };
}
...
async function apiSave(ops) {
  ...
  const r = await fetch(GS.url, {
    ...
    body: JSON.stringify({ token: GS.token, ops }),
  });
  const j = await r.json();
...
function makeQueue(onState) {
  ...
  const flush = async () => {
    ...
    onState("saving");
    try { await apiSave(batch); onState("saved"); }
    catch (e) { onState("error"); pending = batch.concat(pending); }
  };
  return {
    push(ops) {
...
function buildState(state) {
  const weeks = {}, days = {}, notes = {};
  const W = (k) => (weeks[k] = weeks[k] || { meals: {}, bought: {}, extras: [], requests: [], plan: {}, hours: {}, confirm: {}, hidden: {} });
  const D = (k) => (days[k] = days[k] || { done: {} });

  (state.hours || []).forEach((r) => {
    const date = String(r["ngày"]); if (!date) return;
    const wk = fmt(mondayOf(parse(date)));
    const w = W(wk);
    w.hours[date] = { actual: Number(r["phút thực tế"]) || 0, note: String(r["ghi chú"] || "") };
    if (String(r["đã xác nhận"]).trim()) w.confirm[date] = true;
  });

  (state.done || []).forEach((r) => {
    const date = String(r["ngày"]); if (!date) return;
    D(date).done[String(r["việc"])] = { by: String(r["người"] || "") };
  });

  (state.plan || []).forEach((r) => {
    const date = String(r["ngày"]); if (!date) return;
    const w = W(fmt(mondayOf(parse(date))));
    const p = (w.plan[date] = w.plan[date] || { tasks: null, assign: {} });
    if (String(r["người ca"] || "")) p.shiftWho = String(r["người ca"]);
    const task = String(r["việc"] || "");
    if (task === "*") { p.tasks = p.tasks || []; }
    else if (task) {
  ...
  (state.shop || []).forEach((r) => {
    const w = W(String(r["tuần"]));
    const name = String(r["tên"]), src = String(r["nguồn"] || "");
    const bought = String(r["đã mua"]).trim();
    if (src === "ẩn") { w.hidden["m:" + name] = true; return; }
    if (src === "việc") { w.requests.push({ id: name, text: name, done: !!bought }); return; }
    if (src === "tay") w.extras.push({ id: name, name, cat: String(r["nhóm"] || "Khác") });
    if (bought) w.bought[(src === "tay" ? "e:" : "m:") + name] = true;
  });

  (state.notes || []).forEach((r) => {
    const k = String(r["việc"] || "");
    if (k.indexOf("ngày:") === 0) D(k.slice(5)).note = String(r["nội dung"] || "");
    else if (k) notes[k] = String(r["nội dung"] || "");
  });

  return { weeks, days, notes };
}
...
const ops = {
  hours: (date, who, actual, confirmed, note) => [{
    sheet: "Giờ công",
    row: { "ngày": date, "người": who, "phút thực tế": actual, "đã xác nhận": confirmed ? "x" : "", "ghi chú": note || "" },
  }],
  done: (date, taskId, who, on) => [
    on ? { sheet: "Đã làm", row: { "ngày": date, "việc": taskId, "người": who, "lúc": new Date().toISOString() } }
       : { sheet: "Đã làm", row: { "ngày": date, "việc": taskId }, remove: true },
  ],
  plan: (date, tasks, assign, shiftWho) => {
    const out = [{ sheet: "Phân công", clear: { "ngày": date } }];
    if (!tasks) return out;
    if (!tasks.length) out.push({ sheet: "Phân công", row: { "ngày": date, "việc": "*", "người": "", "người ca": shiftWho || "" } });
    tasks.forEach((t) => out.push({
      sheet: "Phân công",
      row: { "ngày": date, "việc": t, "người": assign[t] || "", "người ca": shiftWho || "" },
    }));
    return out;
  },
  meals: (wk, dow, ids) => {
    const out = [{ sheet: "Thực đơn", clear: { "tuần": wk, "thứ": dow } }];
    (ids || []).forEach((id) => out.push({ sheet: "Thực đơn", row: { "tuần": wk, "thứ": dow, "món": id } }));
    return out;
  },
  shop: (wk, name, group, source, bought) => [{
    sheet: "Đi chợ",
    row: { "tuần": wk, "tên": name, "nhóm": group || "", "nguồn": source, "đã mua": bought ? "x" : "" },
  }],
  shopDel: (wk, name) => [{ sheet: "Đi chợ", row: { "tuần": wk, "tên": name }, remove: true }],
  note: (key, text) => [
    text ? { sheet: "Hướng dẫn", row: { "việc": key, "nội dung": text } }
         : { sheet: "Hướng dẫn", row: { "việc": key }, remove: true },
  ],
  recipe: (r) => [{
    sheet: "Món ăn",
    row: { "id": r.id, "tên": r.name, "loại": r.type, "nguyên liệu": r.ing.join(", ") },
  }],
};
...
function Gate({ onEnter }) {
  ...
  const press = (d) => {
    ...
    const next = pin + d;
    setPin(next); setBad(false);
    if (next.length === 4) {
      setTimeout(() => {
        if (next === PINS[pick]) onEnter(pick);
    ...
    <>
      ...
      <div className="app gate">
        <div className="gate-inner">
          <div className="gate-brand">Việc nhà</div>

          {!pick ? (
            <>
              <div className="gate-q">Tên bạn là gì?</div>
              <div className="gate-people">
                {["diem", "lich", "ban"].map((k) => (
                  <button key={k} className="gate-person"
                    style={{ background: PEOPLE[k].color, color: PEOPLE[k].fg }}
                    onClick={() => { setPick(k); setPin(""); setBad(false); }}>
                    <span className="gp-short">{PEOPLE[k].short}</span>
                    <span className="gp-name">{PEOPLE[k].name}</span>
                  </button>
                ))}
              </div>
            </>
            ...
            <>
              <button className="gate-back" onClick={() => { setPick(null); setPin(""); setBad(false); }}>
                ‹ Đổi tên
              </button>
              <div className="gate-who">
                <span className="avatar" style={{ background: PEOPLE[pick].color, color: PEOPLE[pick].fg }}>
                  {PEOPLE[pick].short}
                </span>
                <span className="gate-whoname">{PEOPLE[pick].name}</span>
              </div>
              <div className="gate-q sm">Nhập mã PIN</div>

              <div className={"dots" + (bad ? " bad" : "")}>
                {[0, 1, 2, 3].map((i) => <span key={i} className={"dot" + (pin.length > i ? " on" : "")} />)}
              </div>
              {bad && <div className="gate-err">Mã PIN không đúng, thử lại</div>}
              ...
              <div className="keypad">
                ...
                <button className="kdel" onClick={() => { setPin(pin.slice(0, -1)); setBad(false); }}>⌫</button>
              </div>
            </>
          )}
        </div>
  ...
  return (
    <div className="meter-wrap">
      <div className="meter-track">
        <div className="meter-fill" style={{ width: pct + "%", background: over ? "var(--clay)" : "var(--jade)" }} />
        {Array.from({ length: Math.max(0, Math.floor(cap / 15) - 1) }).map((_, i) => (
          <div key={i} className="meter-tick" style={{ left: ((i + 1) * 15 / cap) * 100 + "%" }} />
        ))}
      </div>
...
function Donut({ pct, size = 76 }) {
  const r = (size - 13) / 2, c = 2 * Math.PI * r;
  const col = pct >= 100 ? "var(--jade)" : pct >= 50 ? "var(--jade)" : pct > 0 ? "var(--amber)" : "var(--line)";
  return (
    <div className="donut" style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="var(--brand-soft)" strokeWidth="10" />
      ...
      <div className="donut-val">{pct}<span>%</span></div>
    </div>
  );
}
...
const ICONS = {
  today: (
    <>
      ...
      <path d="M4 5.5l1 1L7 4" /><path d="M4 12.5l1 1L7 10" /><path d="M4 19.5l1 1L7 17" />
    </>
  ),
  meals: (
    <>
      <path d="M6 3v8a2 2 0 0 0 4 0V3" /><path d="M8 11v10" />
      <path d="M17 3c-1.5 1.5-2 3.5-2 5.5S15.5 12 17 12s2-1.5 2-3.5S18.5 4.5 17 3z" />
      <path d="M17 12v9" />
    </>
  ),
  shop: (
    <>
      ...
      <path d="M2.5 3h2.2l2.4 12.1a1.6 1.6 0 0 0 1.6 1.3h8.7a1.6 1.6 0 0 0 1.6-1.3L21 7H6" />
    </>
  ),
  week: (
    <>
...
function WeekStrip({ today, setToday, week, me, isOwner }) {
  ...
  const days = Array.from({ length: 7 }, (_, i) => {
    ...
    const { tasks, assign } = planFor(week, d);
  ...
  return (
    <div className="strip">
      {days.map((x) => {
        ...
        const p = x.people;
        const bg = p.length === 1 ? PEOPLE[p[0]].color
          : p.length > 1 ? `linear-gradient(135deg, ${PEOPLE[p[0]].color} 50%, ${PEOPLE[p[1]].color} 50%)`
            : "transparent";
        return (
          <button key={x.key} className={"sday" + (sel ? " sel" : "") + (p.length ? " work" : "")}
            onClick={() => isOwner && setToday(x.key)} disabled={!isOwner}>
            <span className="sday-d">{DAY_VN_SHORT[x.d.getDay()]}</span>
            <span className="sday-n" style={p.length ? { background: bg, color: "#fff" } : {}}>
              {x.d.getDate()}
            </span>
            <span className={"sday-dot" + (isToday ? " on" : "")} />
          </button>
        );
      })}
    </div>
...
/* ────────────────────────────────────────────────────────────
   TAB: HÔM NAY
   ──────────────────────────────────────────────────────────── */
function TodayTab({ today, setToday, dayData, toggle, meta, setNote, week, recipes, me, isOwner, dayNote, setDayNote }) {
  const [noteFor, setNoteFor] = useState(null);
  const [draft, setDraft] = useState("");
  const [dn, setDn] = useState(dayNote);
  useEffect(() => { setDn(dayNote); }, [dayNote, today]);
  const d = parse(today);
  const dow = d.getDay();
  const shift = SHIFTS[dow];
  const whoKey = whoFor(week, d);
  const { tasks: allTasks, assign } = planFor(week, d);
  const { actual } = hoursFor(week, d);
  ...
  const openNote = (t) => { setNoteFor(t); setDraft(meta.notes?.[t.id] || ""); };
  const saveNote = () => { setNote(noteFor.id, draft.trim()); setNoteFor(null); };
  ...
  const Row = ({ t }) => {
    ...
    return (
      <div className={"row" + (on ? " done" : "")}>
        <Check on={on} who={done[t.id]?.by} onClick={() => toggle(t)} />
        <div className="row-body" onClick={() => toggle(t)}>
          <div className="row-name">{t.name}</div>
          <div className="row-meta">
            <span className="pill">{t.room}</span>
            {assign[t.id] && assign[t.id] !== whoKey && (
              <span className="own" style={{ background: owner.color, color: owner.fg }}>{owner.name}</span>
            )}
          </div>
          {note && <div className="note">✎ {note}</div>}
        </div>
        {isOwner && (
          <button className="notebtn" onClick={() => openNote(t)}
            aria-label={"Ghi chú cho " + t.name}>✎</button>
        )}
      </div>
    );
  };
  ...
  return (
    <div className="tab">
      <WeekStrip today={today} setToday={setToday} week={week} me={me} isOwner={isOwner} />

      {isOwner && (
        <div className="legend">
          {["diem", "lich"].map((k) => (
            <span key={k} className="lg">
              <i style={{ background: PEOPLE[k].color }} />{PEOPLE[k].name}
            </span>
          ))}
        </div>
      )}
      ...
      <div className="daynav">
        {isOwner && <button onClick={() => setToday(fmt(addDays(d, -1)))}>‹</button>}
        <div>
          <div className="daynav-date">{prettyDate(d)}</div>
          {isOwner && today !== fmt(new Date()) && (
            <button className="link" onClick={() => setToday(fmt(new Date()))}>về hôm nay</button>
          )}
        </div>
        {isOwner && <button onClick={() => setToday(fmt(addDays(d, 1)))}>›</button>}
      </div>
      ...
      {rest ? (
        <div className="rest-card">
          <div className="rest-title">Ngày nghỉ</div>
          <div className="rest-sub">Không có việc nhà được giao.</div>
          {dinner.length > 0 && (
            <div className="rest-meal">
              <div className="eyebrow">Bữa tối</div>
              {dinner.map((r) => <div key={r.id} className="dish-line">{r.name}</div>)}
            </div>
          )}
        </div>
      ) : (
        <>
          <div className="shift-card">
            <div className="shift-top">
              <span className="avatar" style={{ background: person.color, color: person.fg }}>{person.short}</span>
              <div>
                <div className="shift-name">{person.name}</div>
                <div className="shift-cap">
                  {isOwner || whoKey === me
                    ? `Ca ${hm(actual)}${actual !== shift.cap ? " · đã sửa" : ""}`
                    : `${tasks.length} việc được giao · ${hm(capBase)}`}
                </div>
              </div>
            </div>
            <Meter used={used} cap={capBase || shift.cap} />
          </div>

          {dinner.length > 0 && (
            <div className="dinner-strip">
              <div className="eyebrow">Bữa tối hôm nay</div>
              {dinner.map((r) => (
                <div key={r.id} className="dish-line"><b>{r.type}</b> {r.name}</div>
              ))}
            </div>
          )}

          <Section title="Việc hàng ngày" total={daily.reduce((s, t) => s + t.min, 0)}>
            {daily.map((t) => <Row key={t.id} t={t} />)}
          </Section>

          {weekly.length > 0 && (
            <Section title="Việc hàng tuần" total={weekly.reduce((s, t) => s + t.min, 0)}>
              {weekly.map((t) => <Row key={t.id} t={t} />)}
            </Section>
          )}

          {monthly.length > 0 && (
            <Section title={`Việc hàng tháng · tuần ${weekOfMonth(d)}`} total={monthly.reduce((s, t) => s + t.min, 0)}>
              {monthly.map((t) => <Row key={t.id} t={t} />)}
            </Section>
          )}

          {weekOfMonth(d) === 5 && (dow === 1 || dow === 2) && (
            <div className="hint">Tuần thứ 5 của tháng — không có việc hàng tháng.</div>
          )}

          <div className="section">
            <div className="section-head"><span className="eyebrow">Ghi chú gửi BB</span></div>
            <textarea className="daynote" rows={2} value={dn}
              onChange={(e) => setDn(e.target.value)} onBlur={() => setDayNote(dn)}
      ...
      {noteFor && (
        <div className="sheet-bg" onClick={() => setNoteFor(null)}>
          <div className="sheet" onClick={(e) => e.stopPropagation()}>
            <div className="sheet-title">{noteFor.name}</div>
            <div className="eyebrow">Hướng dẫn cố định — hiện mỗi lần làm việc này</div>
            <textarea value={draft} onChange={(e) => setDraft(e.target.value)}
              placeholder="Ví dụ: dùng nước lau chuyên dụng cho mặt đá" rows={4} />
            <div className="sheet-actions">
              <button className="btn ghost" onClick={() => setNoteFor(null)}>Đóng</button>
              <button className="btn" onClick={saveNote}>Lưu ghi chú</button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <div className="section">
      <div className="section-head">
        ...
        <span className="section-total">{mins(total)}</span>
      </div>
      {children}
    </div>
  );
...
/* ────────────────────────────────────────────────────────────
   TAB: THỰC ĐƠN
   ──────────────────────────────────────────────────────────── */
function IngredientInput({ chips, setChips, pool }) {
  const [q, setQ] = useState("");
  const norm = (s) => s.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
  const sugg = useMemo(() => {
    const t = norm(q.trim());
    return pool
      .filter((i) => !chips.includes(i))
      .filter((i) => (t ? norm(i).includes(t) : true))
      .slice(0, t ? 8 : 12);
  }, [q, chips, pool]);
  const exact = pool.some((i) => norm(i) === norm(q.trim()));

  const add = (v) => { const x = v.trim(); if (x && !chips.includes(x)) setChips([...chips, x]); setQ(""); };
  ...
  return (
    <div className="ingbox">
      {chips.length > 0 && (
        <div className="chips">
          {chips.map((c) => (
            <button key={c} className="chip" onClick={() => setChips(chips.filter((x) => x !== c))}>
      ...
      <div className="sugg">
        {q.trim() && !exact && (
          <button className="sugg-btn new" onClick={() => add(q)}>+ Thêm mới “{q.trim()}”</button>
        )}
        {sugg.map((s) => (
          <button key={s} className="sugg-btn" onClick={() => add(s)}>{s}</button>
        ))}
      </div>
    </div>
  );
...
function MealsTab({ week, setWeek, weekStart, shiftWeek, recipes, setRecipes, isOwner }) {
  const [picker, setPicker] = useState(null);
  const [tab, setTab] = useState("Món chính");
  const [q, setQ] = useState("");
  const [adding, setAdding] = useState(false);
  const [nn, setNn] = useState(""); const [ni, setNi] = useState([]); const [nt, setNt] = useState("Món chính");
  ...
  const pick = (dow, id) => {
    ...
    setWeek({ ...week, meals: { ...meals, [dow]: next } }, ops.meals(fmt(weekStart), dow, next));
  };

  const addRecipe = () => {
    if (!nn.trim()) return;
    const r = { id: "u" + Date.now(), name: nn.trim(), type: nt, ing: ni };
    setRecipes([...recipes, r]);
    setNn(""); setNi([]); setAdding(false);
  };
  ...
  return (
    <div className="tab">
      <div className="weeknav">
        ...
        <span>Tuần {weekStart.getDate()}/{weekStart.getMonth() + 1} – {addDays(weekStart, 6).getDate()}/{addDays(weekStart, 6).getMonth() + 1}</span>
        <button onClick={() => shiftWeek(1)}>›</button>
      </div>

      {!isOwner && <div className="ro-note">Thực đơn do BB lên. Cần mua gì thêm, ghi ở tab Đi chợ.</div>}

      {days.map((d) => {
        const dow = d.getDay();
        const chosen = (meals[dow] || []).map((id) => recipes.find((r) => r.id === id)).filter(Boolean);
        return (
          <div key={dow} className={"mealday" + (isOwner ? "" : " ro")}
            onClick={() => isOwner && setPicker(dow)}>
            <div className="mealday-head">
              <span className="dow">{DAY_VN_SHORT[dow]}</span>
              <span className="mealday-date">{d.getDate()}/{d.getMonth() + 1}</span>
            </div>
            <div className="mealday-body">
              {chosen.length === 0
                ? <span className="empty">{isOwner ? "Chạm để chọn món" : "Chưa có món"}</span>
                : chosen.map((r) => (
                  <div key={r.id} className="dish-chip"><span className="dish-type">{r.type}</span>{r.name}</div>
                ))}
            </div>
          </div>
        );
      ...
      {picker !== null && (
        <div className="sheet-bg" onClick={() => setPicker(null)}>
          <div className="sheet tall" onClick={(e) => e.stopPropagation()}>
            <div className="sheet-title">Chọn món · {DAY_VN[picker]}</div>
            <div className="types">
              {TYPES.map((t) => (
                <button key={t} className={"type-btn" + (tab === t ? " on" : "")} onClick={() => setTab(t)}>{t}</button>
              ))}
            </div>
            <input className="search" value={q} onChange={(e) => setQ(e.target.value)} placeholder="Tìm món…" />
            <div className="picklist">
              {list.map((r) => {
                const mine = (meals[picker] || []).includes(r.id);
                const usedElse = usedThisWeek.has(r.id) && !mine;
                return (
                  <button key={r.id} className={"pick" + (mine ? " on" : "") + (usedElse ? " used" : "")}
                    onClick={() => !usedElse && pick(picker, r.id)} disabled={usedElse}>
                    <span>{r.name}</span>
                    {usedElse
                      ? <span className="used-tag">đã dùng tuần này</span>
                      : <span className="ing">{r.ing.join(", ")}</span>}
                  </button>
                );
              })}
              {list.length === 0 && <div className="empty pad">Không có món nào.</div>}
            </div>

            {adding ? (
              <div className="addbox">
                <input value={nn} onChange={(e) => setNn(e.target.value)} placeholder="Tên món" />
                <div className="eyebrow mt">Nguyên liệu</div>
                <IngredientInput chips={ni} setChips={setNi} pool={ingPool} />
                <select value={nt} onChange={(e) => setNt(e.target.value)}>
                  {TYPES.map((t) => <option key={t}>{t}</option>)}
                </select>
                <div className="sheet-actions">
                  <button className="btn ghost" onClick={() => setAdding(false)}>Huỷ</button>
                  <button className="btn" onClick={addRecipe}>Thêm món</button>
                </div>
              </div>
            ) : (
              <div className="sheet-actions">
                <button className="btn ghost" onClick={() => setAdding(true)}>+ Món mới</button>
                <button className="btn" onClick={() => setPicker(null)}>Xong</button>
              </div>
            )}
          </div>
        </div>
...
/* ────────────────────────────────────────────────────────────
   TAB: ĐI CHỢ
   ──────────────────────────────────────────────────────────── */
function GroceryTab({ week, setWeek, recipes, isOwner, weekStart }) {
  ...
  const fromMeals = useMemo(() => {
    ...
    Object.values(week.meals || {}).forEach((arr) => (arr || []).forEach((id) => {
      ...
      if (!r) return;
      r.ing.forEach((i) => { map[i] = map[i] || { name: i, cat: catOf(i), for: [] }; map[i].for.push(r.name); });
    }));
    return Object.values(map);
  }, [week.meals, recipes]);

  const items = [
    ...fromMeals.map((x) => ({ ...x, key: "m:" + x.name, auto: true })),
    ...extras.map((x) => ({ name: x.name, cat: x.cat || "Khác", key: "e:" + x.id, auto: false, id: x.id })),
  ].filter((i) => !hidden[i.key]);

  const hiddenCount = Object.values(hidden).filter(Boolean).length;

  const grouped = CAT_ORDER.map((c) => ({ cat: c, items: items.filter((i) => i.cat === c && !bought[i.key]) })).filter((g) => g.items.length);
  const boughtList = items.filter((i) => bought[i.key]);

  const toggleBuy = (k) => {
    const it = items.find((x) => x.key === k);
    setWeek({ ...week, bought: { ...bought, [k]: !bought[k] } },
  ...
  const removeItem = (i) => {
    const nb = { ...bought }; delete nb[i.key];
    if (i.auto) setWeek({ ...week, bought: nb, hidden: { ...hidden, [i.key]: true } },
      ops.shop(wk, i.name, i.cat, "ẩn", false));
    else setWeek({ ...week, bought: nb, extras: extras.filter((e) => e.id !== i.id) },
      ops.shopDel(wk, i.name));
  };
  const restoreHidden = () => {
    const names = Object.keys(hidden).filter((k) => hidden[k]).map((k) => k.slice(2));
    setWeek({ ...week, hidden: {} }, names.map((n) => ({ sheet: "Đi chợ", row: { "tuần": wk, "tên": n }, remove: true })));
  };
  const addExtra = () => {
    const n = txt.trim(); if (!n) return;
    setWeek({ ...week, extras: [...extras, { id: n, name: n, cat: "Khác" }] }, ops.shop(wk, n, "Khác", "tay", false));
    setTxt("");
  };
  const addStaple = (s) => setWeek({ ...week, extras: [...extras, { id: s.n, name: s.n, cat: s.c }] },
    ops.shop(wk, s.n, s.c, "tay", false));
  const addReq = () => {
    const t = rtxt.trim(); if (!t) return;
    setWeek({ ...week, requests: [...requests, { id: t, text: t, done: false }] }, ops.shop(wk, t, "", "việc", false));
    setRtxt("");
  };
  const toggleReq = (id) => {
    const r0 = requests.find((r) => r.id === id);
    setWeek({ ...week, requests: requests.map((r) => r.id === id ? { ...r, done: !r.done } : r) },
      r0 ? ops.shop(wk, r0.text, "", "việc", !r0.done) : null);
  };

  return (
    <div className="tab">
      <div className="shopweek">
        Tuần {weekStart.getDate()}/{weekStart.getMonth() + 1} – {addDays(weekStart, 6).getDate()}/{addDays(weekStart, 6).getMonth() + 1}
        <span className="muted">· danh sách mới mỗi thứ hai</span>
      </div>

      <div className="addrow">
        <input value={txt} onChange={(e) => setTxt(e.target.value)} placeholder="Thêm món cần mua…"
          onKeyDown={(e) => e.key === "Enter" && addExtra()} />
        <button className="btn sm" onClick={addExtra}>Thêm</button>
      </div>

      {grouped.map((g) => (
        <div key={g.cat} className="section">
          <div className="section-head"><span className="eyebrow">{g.cat}</span><span className="section-total">{g.items.length}</span></div>
          {g.items.map((i) => (
            <div key={i.key} className="row">
              <Check on={false} onClick={() => toggleBuy(i.key)} />
              <div className="row-body" onClick={() => toggleBuy(i.key)}>
                <div className="row-name">{i.name}{!i.auto && <span className="manual">tự thêm</span>}</div>
                {i.for && <div className="row-meta"><span className="ago">{i.for.join(" · ")}</span></div>}
              </div>
              {isOwner && <button className="del" onClick={() => removeItem(i)} aria-label={"Xoá " + i.name}><TrashIcon /></button>}
            </div>
          ))}
        </div>
      ))}

      {grouped.length === 0 && <div className="hint">Chưa có gì trong danh sách. Chọn món ở tab Thực đơn để tự động tạo danh sách đi chợ.</div>}

      {hiddenCount > 0 && isOwner && (
        <div className="restore">
          Đã ẩn {hiddenCount} nguyên liệu từ thực đơn
          <button className="link" onClick={restoreHidden}>hiện lại</button>
        </div>
      )}

      {STAPLE_GROUPS.map((g) => (
        <div key={g.title} className="section">
          <div className="section-head"><span className="eyebrow">{g.title}</span></div>
          <div className="staples">
            {g.items.map((s) => (
              <button key={s.n} className="staple" onClick={() => addStaple(s)}>+ {s.n}</button>
            ))}
          </div>
        </div>
      ))}
      ...
      <div className="section">
        <div className="section-head"><span className="eyebrow">Đồ cần mua khác</span></div>
        <div className="addrow">
          <input value={rtxt} onChange={(e) => setRtxt(e.target.value)} placeholder="Ví dụ: cây lau nhà mới"
            onKeyDown={(e) => e.key === "Enter" && addReq()} />
          <button className="btn sm" onClick={addReq}>Thêm</button>
        </div>
        {requests.map((r) => (
          <div key={r.id} className={"row" + (r.done ? " done" : "")}>
            <Check on={r.done} onClick={() => toggleReq(r.id)} />
            <div className="row-body"><div className="row-name">{r.text}</div></div>
            {isOwner && (
              <button className="del" onClick={() => setWeek({ ...week, requests: requests.filter((x) => x.id !== r.id) }, ops.shopDel(wk, r.text))}
                aria-label={"Xoá " + r.text}><TrashIcon /></button>
            )}
          </div>
        ))}
      </div>
      ...
      {boughtList.length > 0 && (
        <div className="section">
          <div className="section-head">
            <span className="eyebrow">Đã mua</span>
            <span className="section-total">{boughtList.length}</span>
          </div>
          {boughtList.map((i) => (
            <div key={i.key} className="row done">
              <Check on onClick={() => toggleBuy(i.key)} />
              <div className="row-body"><div className="row-name">{i.name}</div></div>
              {isOwner && <button className="del" onClick={() => removeItem(i)} aria-label={"Xoá " + i.name}><TrashIcon /></button>}
            </div>
          ))}
        </div>
...
/* ────────────────────────────────────────────────────────────
   TAB: TUẦN & LỊCH SỬ
   ──────────────────────────────────────────────────────────── */
function WeekTab({ weekStart, shiftWeek, weekDays, recipes, week, setWeek }) {
  const [editing, setEditing] = useState(null); // ISO date
  const days = Array.from({ length: 7 }, (_, i) => addDays(weekStart, i));
  const confirm = week.confirm || {};
  let ticked = 0, totAll = 0, minPlan = 0, daysActive = 0, daysOk = 0;
  const byPerson = {
    diem: { actual: 0, std: 0, days: [], ok: 0, n: 0 },
    lich: { actual: 0, std: 0, days: [], ok: 0, n: 0 },
  };

  const rows = days.map((d) => {
    const key = fmt(d);
    const { tasks, assign, custom } = planFor(week, d);
    const { actual, note, cap } = hoursFor(week, d);
    ...
    ticked += nd; totAll += tasks.length; minPlan += mp;
    if (active) { daysActive += 1; if (ok) daysOk += 1; }
    if (byPerson[whoKey] && active) {
      byPerson[whoKey].actual += actual; byPerson[whoKey].std += cap;
      byPerson[whoKey].n += 1; if (ok) byPerson[whoKey].ok += 1;
      byPerson[whoKey].days.push({ d: DAY_VN_SHORT[d.getDay()], m: actual, cap, ok });
    }
    return {
      d, key, tasks, assign, custom, done, nd, mp, actual, note, cap,
      whoKey, who: PEOPLE[whoKey], active, ok,
    };
  });
  ...
  const toggleConfirm = (r) => {
    ...
    setWeek({ ...week, confirm: c },
      ops.hours(r.key, r.whoKey, h.actual != null ? h.actual : r.cap, on, h.note || ""));
  };
  const setPlan = (key, patch) => {
    const plan = { ...(week.plan || {}) };
    plan[key] = { ...(plan[key] || {}), ...patch };
    const p = plan[key];
    setWeek({ ...week, plan }, ops.plan(key, p.tasks, p.assign || {}, p.shiftWho));
  };
  const resetPlan = (key) => {
    const plan = { ...(week.plan || {}) };
    delete plan[key];
    setWeek({ ...week, plan }, [{ sheet: "Phân công", clear: { "ngày": key } }]);
  };
  ...
  return (
    <div className="tab">
      ...
      <div className="topsum">
        <Donut pct={pct} size={92} />
        <div className="topsum-side">
          <div className="eyebrow">Bạn đã xác nhận</div>
          <div className="topsum-line">{daysOk}/{daysActive} ngày</div>
          <div className="topsum-sub">{hm(actualTot)} giờ công · Lịch/Diễm tự tick {ticked}/{totAll} việc</div>
        </div>
      </div>

      <div className="section">
        <div className="section-head">
          <span className="eyebrow">Giờ công theo người</span>
          <span className="section-total">để tính lương</span>
        </div>
        {["diem", "lich"].map((k) => {
          const p = byPerson[k];
          ...
          return (
            <div key={k} className="pcard">
              <div className="pcard-top">
                <span className="avatar sm" style={{ background: PEOPLE[k].color, color: PEOPLE[k].fg }}>{PEOPLE[k].short}</span>
                <span className="pname">{PEOPLE[k].name}</span>
                <div className="pstats">
                  <div className="pbig">{hm(p.actual)}</div>
                  <div className="psub">
                    chuẩn {hm(p.std)}
                    {diff !== 0 && <span className={diff > 0 ? "over" : "under"}> · {diff > 0 ? "+" : ""}{diff}′</span>}
                  </div>
                </div>
              </div>
              {p.days.length ? (
                <>
                  <div className="dgrid">
                    {p.days.map((x, idx) => (
                      <div key={idx} className={"dcol" + (x.m !== x.cap ? " off" : "") + (x.ok ? " ok" : "")}
                        title={(x.ok ? "Đã xác nhận" : "Chưa xác nhận") + (x.m !== x.cap ? " · khác ca chuẩn" : "")}>
                        <span className="dcol-d">{x.d}{x.ok ? " ✓" : ""}</span>
                        <span className="dcol-t">{x.m !== x.cap ? "≠ " : ""}{hm(x.m)}</span>
                      </div>
                    ))}
                  </div>
                  <div className="psub pad">{p.ok}/{p.n} ca đã xác nhận</div>
                </>
              ) : <div className="psub pad">Không có ca tuần này</div>}
            </div>
          );
        })}
      </div>
      ...
      {rows.map((r) => (
        <div key={r.key} className={"wday" + (r.ok ? " finished" : "")}>
          <div className="wday-head">
            <span className="dow">{DAY_VN_SHORT[r.d.getDay()]}</span>
            <span className="wday-date">{r.d.getDate()}/{r.d.getMonth() + 1}</span>
            {!r.active
              ? <span className="tag-rest">nghỉ</span>
              : <span className="avatar sm" style={{ background: r.who.color, color: r.who.fg }}>{r.who.short}</span>}
            {r.custom && <span className="tag-custom">đã sửa</span>}
            {r.ok && <span className="tag-ok">đã xác nhận</span>}
            <button className="edit-btn" onClick={() => setEditing(r.key)}>Sửa</button>
          </div>

          {r.active && (
            <>
              <div className="hours-row">
                <span className="eyebrow">Giờ thực tế</span>
                <div className="stepper">
                  <button onClick={() => setHours(r.key, { actual: Math.max(0, (Math.ceil(r.actual / 30) - 1) * 30) })}>−</button>
                  <input type="number" value={r.actual} step={30} min={0}
                    onChange={(e) => setHours(r.key, { actual: Math.max(0, Number(e.target.value) || 0) })} />
                  <span className="unit">phút</span>
                  <button onClick={() => setHours(r.key, { actual: (Math.floor(r.actual / 30) + 1) * 30 })}>+</button>
                </div>
              </div>
              <div className="hours-meta">
                <span>{hm(r.actual)}</span>
                {r.actual !== r.cap && (
                  <span className={r.actual > r.cap ? "over" : "under"}>
                    {r.actual > r.cap ? " +" : " "}{r.actual - r.cap}′ so với ca chuẩn {hm(r.cap)}
                  </span>
                )}
                {r.actual === r.cap && <span className="muted"> đúng ca chuẩn</span>}
                <button className="link" onClick={() => setHours(r.key, { actual: r.cap })}>đặt lại</button>
              </div>
              <input className="hnote" value={r.note} placeholder="Ghi chú giờ công (về sớm, làm thêm…)"
                onChange={(e) => setHours(r.key, { note: e.target.value })} />

              <div className="wday-time"><Meter used={r.mp} cap={r.actual || r.cap} /></div>
              <div className="wday-plan">
                {r.tasks.length} việc · kế hoạch {r.mp}′
                {r.mp !== r.actual && (
                  <span className={r.mp > r.actual ? "over" : "under"}>
                    {r.mp > r.actual ? ` · vượt ${r.mp - r.actual}′` : ` · dư ${r.actual - r.mp}′`}
                  </span>
                )}
              </div>
              ...
              <div className="wday-tasks">
                {r.tasks.map((t) => {
                  ...
                  return (
                    <span key={t.id} className={"mini" + (r.done[t.id] ? " on" : "")}>
                      {r.done[t.id] ? "✓ " : ""}{t.name}
                      {owner && owner !== r.whoKey && (
                        <b style={{ color: PEOPLE[owner].deep }}> {PEOPLE[owner].short}</b>
                      )}
                    </span>
                  );
                })}
                {r.tasks.length === 0 && <span className="empty">Chưa giao việc nào</span>}
              </div>

              {weekDays[r.key]?.note && (
                <div className="daynote-show">✎ {r.who.name}: {weekDays[r.key].note}</div>
              )}

              {r.tasks.length > 0 && (
                <>
                  <div className="tickinfo">
                    {r.who.name} đã tự tick {r.nd}/{r.tasks.length} việc
                    <span className="muted"> — chỉ để tham khảo</span>
                  </div>
                  <button className={"done-btn" + (r.ok ? " on" : "")} onClick={() => toggleConfirm(r)}>
                    {r.ok ? "✓ Đã xác nhận — bỏ xác nhận" : "Xác nhận ngày này hoàn thành"}
                  </button>
                </>
              )}
            </>
          ...
          {!r.active && (
            <div className="wday-plan">Không có người giúp việc. Chạm “Sửa” để giao việc.</div>
          )}
        </div>
      ))}
      ...
      {editRow && (
        <PlanEditor row={editRow} onClose={() => setEditing(null)}
          setPlan={setPlan} resetPlan={resetPlan} />
      )}
    </div>
  );
}
...
function Backup() {
  ...
  const run = async () => {
    setState("working"); setCopied(false);
    try {
      const data = await apiLoad();
      const json = JSON.stringify(data, null, 2);
      setText(json); setState("ready");
      try { await navigator.clipboard.writeText(json); setCopied(true); } catch { }
  ...
  return (
    <div className="section backup">
      <div className="section-head">
        <span className="eyebrow">Sao lưu dữ liệu</span>
        <span className="section-total">bản sao từ Google Sheets</span>
      </div>
      {state !== "ready" ? (
        <>
          <button className="btn wide" onClick={run} disabled={state === "working"}>
            {state === "working" ? "Đang lấy dữ liệu…" : "Xuất toàn bộ dữ liệu"}
          </button>
          {state === "error" && <div className="backup-note err-t">Không lấy được dữ liệu. Thử lại sau.</div>}
          {state === "idle" && (
            <div className="backup-note">
              Nên xuất mỗi cuối tháng. Dán nội dung vào Notes hoặc Google Docs để lưu.
            </div>
          )}
        </>
      ) : (
        <>
          <div className="backup-note ok-t">
            {copied ? "✓ Đã sao chép. Dán vào Notes hoặc Google Docs để lưu." : "Chọn hết nội dung bên dưới rồi sao chép."}
          </div>
          <textarea className="backup-box" readOnly value={text}
            onFocus={(e) => e.target.select()} rows={6} />
          <button className="btn ghost wide" onClick={() => { setState("idle"); setText(""); }}>Đóng</button>
        </>
      )}
    </div>
  );
...
function PlanEditor({ row, onClose, setPlan, resetPlan }) {
  ...
  const groups = [
    { title: "Việc hàng ngày", items: byTime(ALL_TASKS.filter((t) => t.kind === "daily")) },
    { title: "Việc hàng tuần", items: byTime(ALL_TASKS.filter((t) => t.kind === "weekly")) },
    { title: "Việc hàng tháng", items: byTime(ALL_TASKS.filter((t) => t.kind === "monthly")) },
  ];
  ...
  const save = () => { setPlan(row.key, { tasks: ids, assign, shiftWho }); onClose(); };
  const reset = () => { resetPlan(row.key); onClose(); };

  return (
    <div className="sheet-bg" onClick={onClose}>
      <div className="sheet tall" onClick={(e) => e.stopPropagation()}>
        <div className="sheet-title">
          Việc ngày {DAY_VN[row.d.getDay()]} {row.d.getDate()}/{row.d.getMonth() + 1}
        </div>

        <div className="shiftpick">
          <span className="eyebrow">Người làm ca này</span>
          <div className="whopick">
            {["diem", "lich", "ban"].map((k) => (
              <button key={k} className={"wpn" + (shiftWho === k ? " on" : "")}
                style={shiftWho === k ? { background: PEOPLE[k].color, borderColor: PEOPLE[k].color, color: PEOPLE[k].fg } : {}}
                onClick={() => setShiftWho(k)}>{PEOPLE[k].name}</button>
            ))}
          </div>
        </div>
        {shiftWho !== SHIFTS[row.d.getDay()].who && (
          <div className="swap-note">
            Đổi ca: giờ công ngày này sẽ tính cho {PEOPLE[shiftWho].name} thay vì {PEOPLE[SHIFTS[row.d.getDay()].who].name}.
          </div>
        )}

        <div className="editor-bar">
          <span>{ids.length} việc · {total}′</span>
          <span className={total > row.actual ? "over" : "under"}>
            ca {hm(row.actual)} {total > row.actual ? `· vượt ${total - row.actual}′` : `· dư ${row.actual - total}′`}
          </span>
        </div>

        <div className="picklist">
          {groups.map((g) => (
            <div key={g.title} className="egroup">
              <div className="eyebrow">{g.title}</div>
              {g.items.map((t) => {
                const on = ids.includes(t.id);
                const who = assign[t.id] || defWho;
                return (
                  <div key={t.id} className={"erow" + (on ? " on" : "")}>
                    <Check on={on} onClick={() => toggle(t.id)} />
                    <div className="erow-body" onClick={() => toggle(t.id)}>
                      <div className="erow-name">{t.name}</div>
                      <div className="row-meta">
                        <span className="pill">{t.room}</span>
                        <span className="mins">{mins(t.min)}</span>
                        {t.kind === "monthly" && <span className="ago">tuần {t.wk}</span>}
                      </div>
                    </div>
                    {on && (
                      <div className="whopick">
                        {["diem", "lich", "ban"].map((k) => (
                          <button key={k} className={"wp" + (who === k ? " on" : "")}
                            style={who === k ? { background: PEOPLE[k].color, borderColor: PEOPLE[k].color, color: PEOPLE[k].fg } : {}}
                            onClick={() => setWho(t.id, k)}>{PEOPLE[k].short}</button>
                        ))}
                      </div>
                    )}
                  </div>
        ...
        <div className="sheet-actions">
          <button className={"btn ghost" + (askReset ? " danger" : "")}
            onClick={() => (askReset ? reset() : setAskReset(true))}>
            {askReset ? "Xoá tuỳ chỉnh?" : "Về mặc định"}
          </button>
...
export default function App() {
  ...
  const [meta, setMetaState] = useState({ notes: {} });
  ...
  const week = weeks[wKey] || { meals: {}, bought: {}, extras: [], requests: [], plan: {}, hours: {}, confirm: {}, hidden: {} };
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      setDays(built.days);
      setMetaState({ notes: built.notes });
  ...
  const signIn = (k) => { setMe(k); try { localStorage.setItem("vn:me", k); } catch { } };
  const signOut = () => { setMe(null); try { localStorage.removeItem("vn:me"); } catch { } };

  const patchWeek = (w) => setWeeks((p) => ({ ...p, [wKey]: w }));

  // ── ghi ──
  const setWeek = useCallback((w, o) => { patchWeek(w); if (o) push(o); }, [wKey]);
  ...
  const setMeta = useCallback((m, o) => { setMetaState(m); if (o) push(o); }, []);

  const toggle = (t) => {
    const cur = days[today]?.done || {};
    const on = !!cur[t.id];
    const nextDone = { ...cur };
    if (on) delete nextDone[t.id]; else nextDone[t.id] = { by: me };
    setDays((p) => ({ ...p, [today]: { ...(p[today] || {}), done: nextDone } }));
    push(ops.done(today, t.id, me, !on));
  };

  const setDayNote = (text) => {
    setDays((p) => ({ ...p, [today]: { ...(p[today] || { done: {} }), note: text } }));
  ...
  const setNote = (id, text) => {
    ...
    setMeta({ ...meta, notes }, ops.note(id, text));
  };
  ...
  if (checking || loading) return <div className="loading">Đang tải…</div>;
  if (err) return (
    <><style>{CSS}</style><div className="app gate"><div className="gate-inner">
      <div className="gate-brand">Việc nhà</div>
      <div className="setup-err">{err}</div>
      <button className="btn wide" onClick={() => { setLoading(true); loadAll(); }}>Thử lại</button>
    </div></div></>
  );
  if (!ready) return <div className="loading">Đang tải…</div>;
  if (!me) return <Gate onEnter={signIn} />;
    ...
    <>
      ...
      <div className="app">
        <header className="hdr">
          <div className="brand">
            <span className="brand-icon"><TabIcon name={tab} /></span>
            <span>{TAB_LABEL[tab]}</span>
          </div>
          <button className="refresh" onClick={() => { queue.current.flush(); loadAll(); }}
            title="Tải lại từ Google Sheets" aria-label="Tải lại">⟳</button>
          <button className="whoami" onClick={signOut} title="Đổi người">
            <span className="whoami-n">{PEOPLE[me].name}</span>
            <span className="avatar sm" style={{ background: PEOPLE[me].color, color: PEOPLE[me].fg }}>
              {PEOPLE[me].short}
            </span>
          </button>
        </header>
        ...
        {sync !== "saved" && (
          <div className={"syncbar " + sync}>
            {sync === "saving" ? "Đang lưu…" : sync === "error"
              ? "⚠ Chưa lưu được — sẽ thử lại. Kiểm tra mạng."
        ...
        <main>
          {tab === "today" && (
            <TodayTab today={today} setToday={setToday} dayData={days[today]} toggle={toggle}
              meta={meta} setNote={setNote} week={week} recipes={recipes} me={me} isOwner={isOwner}
              dayNote={days[today]?.note || ""} setDayNote={setDayNote} />
          )}
          {tab === "meals" && (
            <MealsTab week={week} setWeek={setWeek} weekStart={weekStart} shiftWeek={shiftWeek}
              recipes={recipes} setRecipes={setRecipes} isOwner={isOwner} />
          )}
          {tab === "shop" && <GroceryTab week={week} setWeek={setWeek} recipes={recipes} isOwner={isOwner} weekStart={weekStart} />}
          {tab === "week" && isOwner && (
            <WeekTab weekStart={weekStart} shiftWeek={shiftWeek} weekDays={days}
              recipes={recipes} week={week} setWeek={setWeek} />
          )}
        </main>

        <nav className="tabs">
          {[["today", "Hôm nay"], ["meals", "Thực đơn"], ["shop", "Đi chợ"], ...(isOwner ? [["week", "Tuần"]] : [])].map(([k, l]) => (
